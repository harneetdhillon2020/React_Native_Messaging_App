...
const AddMemberCard = ({members}) => {
  const {addWorkspaceMemberModel, selectedWorkspace} =
  ...
  const handleAddWorkspaceMember = () => {
    addWorkspaceMemberModel(selectedWorkspace, members);
    swipeableRef.current.close();
  };

  const rightSwipeActions = () => {
    return (
      <TouchableWithoutFeedback onPress={() => handleAddWorkspaceMember()}>
        <View>
          <Text style={styles.name} accessibilityLabel={`add ${members.name}`}>
            Add
          </Text>
        </View>
